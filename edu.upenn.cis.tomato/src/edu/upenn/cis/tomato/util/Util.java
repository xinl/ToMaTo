...
public class Util {
    ...
    public static String GetCGNodeOrigin(IR ir, IMethod method) {
        ...
        for (int i = 0; i < instructions.length; i++) {
            ...
            if (pos != null) {
        		...
        		{
                	result = pos.getURL().getFile().split("\\\\");
        		}
                else
                {
                	result = pos.getURL().getFile().split(File.separator);
                }
    ...
    public static ObjectMembers GetLineRangeForCGNode(CGNode node,
        ...
        if (range.size() > 0) {
            ...
            ErrorUtil
                    .ErrorMessage("Unable to get the source code range for function ["
                            + functionName + "].");
        }
        return members;
    }
    ...
    public static void PrintViolationSites(
        ...
        DebugUtil.DEBUG_PrintSeperationLine();
        DebugUtil.DEBUG_PrintDebugMessage("ToMaTo violation sites interface with code instrumentation: \n");
        Iterator<Entry<String, ArrayList<ViolationSite>>> iter = ViolationSites.entrySet().iterator();
        while (iter.hasNext()) {
